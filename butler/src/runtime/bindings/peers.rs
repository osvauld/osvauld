//! Lua peers bindings for headless runtime
//!
...
///
/// **Context**: Node scripts need to check if anyone is listening before broadcasting.
...
impl UserData for PeersBindings {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("count", |_, this, ()| {
            ...
            this.scribe_ref.cast(ScribeMessage::GetSubscriberCount { reply: tx })
                .map_err(|e| mlua::Error::RuntimeError(format!("Failed to send GetSubscriberCount: {}", e)))?;

            let count = block_on_async(async {
