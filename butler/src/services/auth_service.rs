...
pub fn login(store: &RedbStore, passphrase: &str) -> Result<Identity> {
    ...
    let encrypted_keys = EncryptedKeys {
        ...
        public_signing_key: identity_data.signing_public_key.clone().try_into()
            .map_err(|_| ButlerError::Crypto("Invalid public signing key length".into()))?,
        public_encryption_key: identity_data.encryption_public_key.clone().try_into()
            .map_err(|_| ButlerError::Crypto("Invalid public encryption key length".into()))?,
        public_device_key: identity_data.device_public_key.clone().try_into()
            .map_err(|_| ButlerError::Crypto("Invalid public device key length".into()))?,
    ...
    let identity = keystore::decrypt_and_restore(&encrypted_keys, passphrase)
        .map_err(|_| ButlerError::InvalidPassphrase)?;
