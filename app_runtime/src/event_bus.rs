...
mod tests {
    ...
    fn test_basic_emit_subscribe() {
        ...
        assert_eq!(deliveries[0].1, 42); // callback_key
    }
    ...
    fn test_target_filter() {
        ...
        // Event with different target - should not match
        let event1 = Event::new("click", EventSource::Human).with_target("cancel");
        let deliveries1 = bus.emit(event1);
        assert_eq!(deliveries1.len(), 0);

        // Event with matching target - should match
        let event2 = Event::new("click", EventSource::Human).with_target("submit");
