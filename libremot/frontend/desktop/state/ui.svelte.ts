...
class UIState {
  ...
  userSelectionModal = $state<UserSelectionModal>({
    show: false,
    users: [],
  });
  ...
  showUserSelectionModal(users: Array<{id: string; username: string; publicKey: string; isOnline: boolean}>) {
    this.userSelectionModal.users = users;
    this.userSelectionModal.show = true;
  }

  hideUserSelectionModal() {
    this.userSelectionModal.show = false;
    this.userSelectionModal.users = [];
  }
