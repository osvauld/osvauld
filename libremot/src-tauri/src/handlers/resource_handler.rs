...
pub async fn handle_add_resource(
    ...
    let preview_data = match generate_chat_preview(&resource_added.data, &user.id).await {
        Ok(data) => data,
        Err(e) => {
            eprintln!(
                "Failed to generate chat preview for resource {}: {}",
                resource_added.id, e
            );
            Default::default()
        }
    ...
    let resource_preview = ResourcePreview {
        ...
        title: String::new(),  // Not used for chats
        preview: String::new(), // Not used for chats
        ...
        last_message: Some(preview_data.last_message),
        last_message_time: Some(preview_data.last_message_time),
        participant_name: Some(preview_data.participants.join(", ")),
        participant_id: preview_data.participant_ids.first().cloned(),
        unread_count: Some(preview_data.unread_count),
        is_online: Some(false), // TODO: Add online status tracking
    };
...
pub async fn handle_update_resource(
    ...
    // Generate chat preview
    let preview_data = match generate_chat_preview(&decrypted_resource.data, &user.id).await {
        Ok(data) => data,
        Err(e) => {
            eprintln!(
                "Failed to generate chat preview for resource {}: {}",
    ...
    let selected_resource_response = if let Some(selected_resource) = selected_resource_id {
        ...
        .await
        {
            Ok(decrypted_resource) => {
                // Generate chat preview
                let preview_data = match generate_chat_preview(&decrypted_resource.data, &user_id).await {
                    Ok(data) => data,
                    Err(e) => {
                        eprintln!(
                            "Failed to generate chat preview for resource {}: {}",
                ...
                tasks.spawn(async move {
                    ...
                    .await
                    {
                        Ok(decrypted_resource) => {
                            ...
                            // Generate chat preview
                            let preview_data = match generate_chat_preview(&decrypted_resource.data, &user_id_clone).await {
                                Ok(data) => data,
                                Err(e) => {
                                    eprintln!(
                                        "Failed to generate chat preview for resource {}: {}",
