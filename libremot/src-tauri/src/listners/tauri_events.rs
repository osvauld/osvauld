...
impl EventManager {
    ...
    pub(super) fn setup_tauri_listeners(&self) {
        self.setup_chat_sync_update_listener();
    ...
    /// Chat-specific sync-update listener that broadcasts to all subscribers
    /// This replaces the document-centric approach with a subscription-based approach
    fn setup_chat_sync_update_listener(&self) {
        ...
        self.app_handle.listen("sync-update", move |event| {
            ...
            let payload = match Self::parse_update_payload(payload_str, "chat sync update") {
                Some(p) => p,
                None => return,
            };

            tokio::spawn(async move {
                // Apply update to buffer for this resource
                chat_state
                    .apply_update(&payload.resource_id, payload.update_bytes.clone(), &payload.doc_type)
                    .await;

                info!(
                    "Applied {} bytes to chat resource: {}",
                    payload.update_bytes.len(),
                    payload.resource_id
                );

                // Get subscribers for this resource (with caching)
                // These are device IDs in base64 format from the database
                let device_ids = match chat_state.get_subscribers(&payload.resource_id).await {
                    Ok(subs) => subs,
                    Err(e) => {
                        error!("Failed to get subscribers for resource {}: {}", payload.resource_id, e);
                        return;
                    }
                };

                if device_ids.is_empty() {
                    info!("No subscribers for resource: {}", payload.resource_id);
                    return;
                }

                // Convert device IDs (base64 public keys) to NodeIDs (hex format) 
                // that the P2P connection manager expects
                ...
                for device_id in &device_ids {
                    match crypto_utils::derive_node_id_from_public_key(device_id) {
                        ...
                        Err(e) => {
                            error!("Failed to derive node ID from device {}: {}", device_id, e);
                ...
                if connection_ids.is_empty() {
                    error!("No valid connection IDs after conversion");
                    return;
                }

                info!(
                    "Broadcasting chat update to {} connections for resource: {}",
                    connection_ids.len(),
                    payload.resource_id
                );

                // Broadcast to all subscribers (fire-and-forget)
                // The P2P service will filter out inactive connections
                if let Err(e) = p2p_sender.send_sync_update_to_connections(
                    payload.resource_id,
                    payload.client_id,
                    payload.update_bytes,
                    connection_ids,  // Now using proper hex NodeIDs
                    payload.doc_type,
                ) {
                    error!("Failed to broadcast chat update: {}", e);
                } else {
                    info!("Successfully broadcasted chat update");
                }
            });
        });
    }
    ...
    fn setup_note_change_listener(&self) {
        ...
        self.app_handle.listen("note-change", move |event| {
            ...
            info!("Received note-change (chat-change) event with chat_id: {:?}", new_note_id);

            // Single spawn for the entire operation to maintain proper order
            tokio::spawn(async move {
                // Handle opening a new chat
                match new_note_id {
                    None => {
                        // No chat selected
                        chat_state.set_current_chat(None).await;
                        note_state.set_current_note(None, None, None).await;
                        info!("No chat selected");
                    }
                    Some(chat_id) => {
                        // Set as current chat in ChatState
                        chat_state.set_current_chat(Some(chat_id.clone())).await;
                        
                        // Get current user to load the resource and set user context
                        let user_state = app_handle.state::<UserState>();
                        let (current_user, current_device) = match (user_state.get_user().await, user_state.get_device().await) {
                            (Ok(user), Ok(device)) => (user, device),
                            (Err(e), _) | (_, Err(e)) => {
                                error!("Failed to get current user/device: {}", e);
                                return;
                            }
                        };

                        // Set user context in chat state
                        chat_state.set_current_user(current_user.id.clone(), current_device.id.clone()).await;
                        ...
                        .await
                        {
                            Ok(decrypted_resource) => {
                                ...
                                // Load into ChatState buffers
                                chat_state
                                    .load_chat_resource(
                                        chat_id.clone(),
                                        main_doc_state.clone(),
                                        image_state.clone(),
                                    )
                                    .await;

                                // Also set in current_note_state for compatibility
                                note_state
                                    .set_current_note(
                                        Some(chat_id.clone()),
                                        main_doc_state,
                                ...
                                info!("Loaded chat resource into buffers: {}", chat_id);
                            }
                            Err(e) => {
                                error!("Failed to load chat resource {}: {}", chat_id, e);
                                // Still set the chat ID even if loading failed
                                chat_state.set_current_chat(Some(chat_id.clone())).await;
                                note_state
                                    .set_current_note(Some(chat_id.clone()), None, None)
                                    .await;
                            }
                        }

                        // Perform reconciliation with all subscribers
                        Self::reconcile_chat_on_open(
                            chat_state.clone(),
                            chat_id.clone(),
                            p2p_sender.clone(),
                        )
    ...
    /// Note: Currently unused in chat implementation but kept for reference
    #[allow(dead_code)]
    ...
    /// Reconcile chat state when opening a chat
    /// This ensures all missed messages are received via state vector exchange
    async fn reconcile_chat_on_open(
        chat_state: crate::chat_state::ChatState,
        chat_id: String,
        p2p_sender: P2PSender,
    ) {
        info!("Starting chat reconciliation for: {}", chat_id);

        // Get subscribers for this chat (with caching)
        let subscribers = match chat_state.get_subscribers(&chat_id).await {
            Ok(subs) => subs,
            Err(e) => {
                error!("Failed to get subscribers for chat reconciliation {}: {}", chat_id, e);
                return;
            }
        };

        if subscribers.is_empty() {
            info!("No subscribers for chat, skipping reconciliation: {}", chat_id);
            return;
        }

        info!(
            "Reconciling chat with {} subscribers: {}",
            subscribers.len(),
            chat_id
        );

        // Get state vectors for this chat
        let state_vectors = match chat_state.get_state_vectors(&chat_id).await {
            Ok(sv) => sv,
            Err(e) => {
                error!("Failed to get state vectors for reconciliation: {}", e);
                return;
            }
        };

        // Send document check requests to all subscribers (establish connections if needed)
        // Device IDs are used as connection IDs in this architecture
        if let Err(e) = p2p_sender.send_live_edit_requests(subscribers.clone()) {
            error!("Failed to send live edit requests for reconciliation: {}", e);
        }

        // Give a brief moment for connections to establish
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

        // For each subscriber, initiate state vector exchange
        for device_id in subscribers {
            info!("Sending document check to device for reconciliation: {}", device_id);
            
            // Send document check with state vectors (similar to live_edit_connected flow)
            if let Err(e) = p2p_sender.send_live_edit_document_check_response(
                device_id.clone(),
                chat_id.clone(),
                true, // is_match = true to trigger reconciliation
                state_vectors.clone(),
            ) {
                error!(
                    "Failed to send document check for reconciliation to {}: {}",
                    device_id, e
                );
            } else {
                info!("Sent reconciliation request to device: {}", device_id);
            }
        }

        info!("Chat reconciliation initiated for: {}", chat_id);
    }
}
