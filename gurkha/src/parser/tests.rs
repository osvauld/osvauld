...
fn issue_on_preserved_in_page_owner_token() {
    use crate::service::issue_page_owner_token_from_policy;
    use policy_model::PolicyFacts;

    let signing_key = [1u8; 32];
    let page_id = "test-page-123";
    let layer_names = vec![format!("{}/content_doc", page_id)];
    let policy = PolicyFacts::default();

    // Create owner token using the new from_policy API
    let (token, _cid) = pollster::block_on(issue_page_owner_token_from_policy(
        &signing_key,
        page_id,
        &policy,
        &layer_names,
    ...
    // Verify issue_on.node exists (auto-generated by from_policy)
    ...
    // Verify issue_on.viewer exists (auto-generated by from_policy)
    ...
    // Verify issue_on.layer_authority exists (auto-generated by from_policy)
    let _layer_authority_template = permit
        .get_issue_template("layer_authority")
        .expect("Permit should have issue_on.layer_authority template");
}
...
fn test_authorized_peers_list_missing() {
    ...
    let signing_key = [1u8; 32];
    let page_id = "page123";
    let layer_names = vec![format!("{}/content", page_id)];
    let policy = PolicyFacts::default();
...
fn test_authorized_peers_list_explicit() {
    ...
    let (owner_token, _) = pollster::block_on(issue_page_owner_token_from_policy(
        &signing_key,
        page_id,
        &policy,
        &layer_names,
