...
function on_init()
    ...
    config_layer = scribe:map(page_id .. "/sim_config")
    ...
    reset_particles()
    sync_config_ui()
    timer.setInterval(16, physics_tick)
end
...
function physics_tick()
    ...
    for _, p in ipairs(sim.particles) do
        ...
        if p.x - p.radius < 0 then p.x = p.radius; p.vx = -p.vx * sim.bounce
        elseif p.x + p.radius > CANVAS_WIDTH then p.x = CANVAS_WIDTH - p.radius; p.vx = -p.vx * sim.bounce end

        if p.y - p.radius < 0 then p.y = p.radius; p.vy = -p.vy * sim.bounce
        elseif p.y + p.radius > CANVAS_HEIGHT then p.y = CANVAS_HEIGHT - p.radius; p.vy = -p.vy * sim.bounce end
    end
...
function explode()
    local cx, cy = CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2
    for _, p in ipairs(sim.particles) do
...
function on_click(target)
    ...
    elseif target:match("^preset:") then
        apply_preset(target:sub(8))
    end
end

function apply_preset(preset)
    if preset == "rain" then
        sim.gravity, sim.bounce, sim.particle_count = 0.8, 0.3, 200
    elseif preset == "bounce" then
        sim.gravity, sim.bounce, sim.particle_count = 0.5, 0.95, 50
    elseif preset == "float" then
        sim.gravity, sim.bounce, sim.particle_count = 0.1, 0.9, 100
    end
    sync_config_ui()
    reset_particles()
    save_config()
