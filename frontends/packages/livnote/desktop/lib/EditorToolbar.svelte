...
<script>
	...
	const commands = {
		...
		h1: () => {
			console.log("Running h1");
			runCommand(
				setBlockType(editorView.state.schema.nodes.heading, { level: 1 }),
			)();
		},
		h2: () => {
			console.log("Running h2");
			console.log("Schema nodes:", editorView.state.schema.nodes);
			console.log("Heading node:", editorView.state.schema.nodes.heading);
			runCommand(
				setBlockType(editorView.state.schema.nodes.heading, { level: 1 }),
			)();
		},
