...
import { toggleMark, lift, joinUp, setBlockType, wrapIn } from "prosemirror-commands";
...
function addBlockItems(container, schema, view) {
  ...
  if (schema.nodes.heading) {
    for (let level = 1; level <= 3; level++) {
      const headingButton = createButton(`H${level}`, `Heading ${level}`, () => {
...
function addListItems(container, schema, view) {
  ...
  // Lift (outdent)
  const liftButton = createButton("↑ Lift", "Lift out of enclosing block", () => {
