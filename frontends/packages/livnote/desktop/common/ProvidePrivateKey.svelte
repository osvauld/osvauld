<script lang="ts">
	import { CopyIcon } from "@osvauld/password-manager-common";



  let revealKey = $state(false)

	let recoveryData = `sdfds`


</script>

<div class="relative pr-16 flex flex-col justify-center items-center">	<button class="absolute right-0 top-[10rem] cursor-pointer bg-osvauld-fieldActive border border-transparent focus:border-livnotePink outline-0 rounded-lg z-999 p-2.5" onclick={() => navigator.clipboard.writeText(recoveryData)}><CopyIcon color="#85889C"/></button>
	
<div class="h-[343px] w-full sm:w-[600px] md:w-[800px] lg:w-[1000px] xl:w-[1173px] max-w-[1173px] text-osvauld-quarzowhite bg-osvauld-frameblack rounded-lg border border-osvauld-iconblack focus-within:border-livnotePink relative p-1.5 transition-colors duration-300">
	<div
