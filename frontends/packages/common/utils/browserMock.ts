		...
		sendMessage: async ({ action, data }) => {
			try {
				...
				const handlerMap = {
					...
					addCredential: (data) =>
						invoke("handle_add_credential", { input: data }),
					hashAndSign: (data) =>
					...
					getCredentialsForFolder: (data) =>
						invoke("handle_get_credentials_for_folder", { input: data }),
					startP2PListner: () => invoke("start_p2p_listener"),
					deleteCredential: (data: any) =>
						invoke("soft_delete_credential", { input: data }),
					...
					getAllCredentials: (data: any) =>
						invoke("get_all_credentials", { input: data }),
