use crate::{
    EventManager, chat_preview_generator::generate_chat_preview, search_index::SearchIndexManager,
...
impl EventManager {
    ...
    pub(super) async fn emit_resource_preview(
        ...
        let preview_data = match generate_chat_preview(&decrypted_resource.data, &current_user.id).await {
            Ok(data) => data,
            Err(e) => {
                error!(
