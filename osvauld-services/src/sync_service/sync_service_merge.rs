...
impl SyncService {
    ...
    pub async fn process_resource_merge_message(
        ...
        match message {
            ResourceUpdateMsg::StateVectorRequest {
                ...
                let response = self
                    .handle_state_vector_request(resource_id, state_vector)
                    .await?;
                Ok(Some(response))
            }
            ResourceUpdateMsg::UpdatesResponse {
                ...
                let response = self
                    .handle_update_and_return_update(resource_id, updates, state_vector)
                    .await?;
                Ok(Some(response))
            }
    ...
    async fn handle_state_vector_request(
        ...
        let updates = match self
            .resource_service
            .generate_updates_for_peer(resource_id, state_vector)
            .await
        {
            Ok(updates) => {
                debug!(
        ...
        let our_state_vector = match self
            .resource_service
        ...
        info!(updates_size = updates.len(), "Sending updates response");
    ...
    pub async fn get_resource_for_update(
        ...
        let resource_state_vector = match self
            .resource_service
            .get_resource_state_vector(resource_id)
            .await
        {
    ...
    async fn handle_update_and_return_update(
        ...
        match self
            .resource_service
            .apply_updates_and_get_peer_updates(resource_id, updates, state_vector)
            .await
        {
    ...
    async fn merge_vector_clocks(
        ...
        let local_vector_clocks = match self
            .vector_clock_repository
            .get_vector_clocks_for_resource(resource_id)
            .await
        {
