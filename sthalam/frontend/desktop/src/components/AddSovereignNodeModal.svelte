<script lang="ts">
	...
	const handleAddSovereignNode = async (connectionStr: string) => {
		try {
			await sendMessage("addSovereignNode", connectionStr);
	...
	const handleSubmit = async (e: Event) => {
		...
		try {
			...
			console.log(
				"Connection string format validation will be handled by backend",
			);
		}
		...
		<div class="h-full p-1">
			...
			>
				...
				>
					...
					>
						Connect to your sovereign node to publish your websites. The
						sovereign node acts as your personal web server, hosting your sites
						independently. Paste the connection string provided by your
						sovereign node to establish a secure connection. Once connected, you
						can publish and update your websites directly from Sthalam.
					</p>
				</div>
				...
				<form onsubmit={handleSubmit} class="space-y-4">
					<div class="space-y-2">
						...
						<p id="connection-string-help" class="text-xs text-textActive">
							The connection string is provided by your sovereign node and is
							safe to use.
						</p>
					</div>
