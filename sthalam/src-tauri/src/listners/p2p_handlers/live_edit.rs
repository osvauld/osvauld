...
impl EventManager {
    /// Verify that the peer has permission to edit the resource
    /// Returns true if peer has crud/update capability, false for view/public (read-only)
    async fn verify_edit_permission(
        &self,
        resource_id: &str,
        connection_id: &str,
    ) -> Result<bool, String> {
        // TODO: In a full implementation, we would:
        // 1. Get the UCAN token for this connection from the P2P handshake
        // 2. Parse and validate the token
        // 3. Check if it has crud/update capability
        //
        // For now, we'll implement a basic check that can be enhanced later
        // The actual UCAN token is stored during P2P handshake in the network layer

        // This is a placeholder - in production, fetch the actual UCAN from connection metadata
        // For now, we'll allow all connections (maintaining backward compatibility)
        // but log a warning
        warn!(
            "UCAN permission check not fully implemented for connection: {} on resource: {}",
            connection_id, resource_id
        );

        Ok(true) // Allow by default for now
    }
    ...
    pub(crate) async fn handle_document_process_update(
        ...
        // Verify peer has edit permission
        match self.verify_edit_permission(&resource_id, &connection_id).await {
            Ok(has_permission) => {
                if !has_permission {
                    warn!(
                        "Connection {} attempted to edit resource {} without crud/update permission",
                        connection_id, resource_id
                    );
                    return;
                }
            ...
            Err(e) => {
                error!("Failed to verify edit permission: {}", e);
                return;
            }
        }
