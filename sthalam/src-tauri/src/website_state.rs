...
/// Per-resource buffer information for website documents
#[derive(Debug, Clone)]
...
/// Website state managing multiple website resources
#[derive(Clone)]
pub struct WebsiteState {
    ...
    /// Currently open website (for UI context only, not for routing decisions)
    current_website_id: Arc<RwLock<Option<String>>>,
...
impl WebsiteState {
    pub fn new(repo_ctx: Arc<RepositoryContext>) -> Self {
        Self {
            ...
            current_website_id: Arc::new(RwLock::new(None)),
            repo_ctx,
    ...
    /// Set the currently open website (for UI context)
    pub async fn set_current_website(&self, website_id: Option<String>) {
        *self.current_website_id.write().await = website_id.clone();
        info!("Current website set to: {:?}", website_id);
    }

    /// Get the currently open website
    pub async fn get_current_website(&self) -> Option<String> {
        self.current_website_id.read().await.clone()
    }

    /// Load a website resource into buffers
    /// This will create new docs and populate them with the website's state
    pub async fn load_website_resource(
        &self,
        resource_id: String,
        blocksuite_state: Option<Vec<u8>>,
        image_state: Option<Vec<u8>>,
    ) {
        let mut buffers = ResourceBuffers::new(resource_id.clone());

        // Load blocksuite document state if provided
        if let Some(bs_state) = blocksuite_state {
            if !bs_state.is_empty() {
                if let Err(e) = buffers.blocksuite_doc.apply_update_v2(&bs_state).await {
                    error!("Failed to load blocksuite document state: {}", e);
                }
            }
        }
        ...
        info!("Loaded website resource into buffers: {}", resource_id);
    }

    /// Apply updates to a specific website resource
    pub async fn apply_update(&self, resource_id: &str, new_updates: Vec<u8>, doc_type: &str) {
        ...
        let mut temp_doc = if is_image_doc {
            ...
            resource_buffers.blocksuite_doc.clone()
        };
        ...
        if let Some(resource_buffers) = buffers_map.get_mut(resource_id) {
            if is_image_doc {
                ...
                resource_buffers.blocksuite_doc = temp_doc;
            }
        }
    ...
    /// Get state vectors for a specific website resource
    pub async fn get_state_vectors(&self, resource_id: &str) -> Result<String, String> {
        ...
        // Get state vector for blocksuite doc
        let blocksuite_sv = resource_buffers.blocksuite_doc.get_state_vector_v2().await;
        if !blocksuite_sv.is_empty() {
            result.insert(
                "blocksuite_doc".to_string(),
                serde_json::Value::Object(Self::create_doc_result(vec![], blocksuite_sv)),
            );
        }
    ...
    /// Returns the complete state of the blocksuite_doc for a resource
    pub async fn export_document_state(&self, resource_id: &str) -> Result<Vec<u8>, String> {
        ...
        // Export the full state of the blocksuite document
        use yrs::{StateVector, Transact, ReadTxn};
        let txn = resource_buffers.blocksuite_doc.transact();
    ...
    pub async fn generate_updates_for_peer(
        ...
        // Process blocksuite_doc
        Self::process_doc_state_vectors(
            &resource_buffers.blocksuite_doc,
            peer_data.get("blocksuite_doc"),
            "blocksuite_doc",
            &mut result,
        )
        .await?;
    ...
    pub async fn apply_updates_and_generate_diff(
        ...
        // Process blocksuite_doc
        let blocksuite_doc = resource_buffers.blocksuite_doc.clone();
        drop(buffers);
    ...
    pub async fn apply_peer_updates(
        ...
        // Process blocksuite_doc updates
        if let Some(blocksuite_data) = peer_data.get("blocksuite_doc") {
            let peer_updates = Self::parse_byte_array(blocksuite_data.get("updates"));

            if !peer_updates.is_empty() {
                self.apply_update(resource_id, peer_updates, "blocksuite_doc").await;
            }
        }
    ...
    /// Get or fetch subscribers for a website resource (with caching)
    ...
    /// Switch to a different resource, loading it if necessary
    pub async fn switch_resource(
        &self,
        resource_id: &str,
        user_id: &str,
        device_id: &str,
    ) -> Result<(), String> {
        // Set current user/device if not already set
        self.set_current_user(user_id.to_string(), device_id.to_string())
            .await;

        // Check if already loaded
        let buffers_map = self.buffers.read().await;
        if buffers_map.contains_key(resource_id) {
            drop(buffers_map);
            self.set_current_website(Some(resource_id.to_string()))
                .await;
            return Ok(());
        }
        drop(buffers_map);

        // Create empty buffers for this resource
        // The actual state will be loaded by the frontend via handle_get_resource
        self.load_website_resource(resource_id.to_string(), None, None)
            .await;
    ...
    /// Get the current state of a resource as bytes
    pub async fn get_state(&self, resource_id: &str) -> Result<Vec<u8>, String> {
        let buffers_map = self.buffers.read().await;

        if let Some(buffers) = buffers_map.get(resource_id) {
            let state = buffers
                .blocksuite_doc
                .get_state_vector_v2()
                .await;
            ...
            Err(format!("Resource {} not loaded", resource_id))
        }
    }
}
