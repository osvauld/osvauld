...
**Creating a form?** Jump directly to [Form Handling](#form-handling) section.

**Most common mistakes with forms:**
1. ❌ Using `form-submit-button` (doesn't exist anymore)
2. ❌ Forgetting form metadata block
3. ❌ Mismatched `formId` between form and fields
4. ✅ **Solution:** Use `nav-button` with `formId` property for submissions

**Working form example:** See [Complete Working Form Example](#complete-working-form-example)
...
> **⚠️ IMPORTANT FOR FORMS:**
> This is the ONLY way to submit forms in HUML!
> The old `form-submit-button` type no longer exists.
> Always use `nav-button` with `formId` property for form submissions.
...
- `type` - Must be `"nav-button"` (NOT form-submit-button)
...
### ⚠️ CRITICAL: How Forms Work

Forms in HUML require **THREE essential parts in the correct order**:

1. **Form metadata block** (type: `"form"`) - MUST come first
2. **Form field blocks** (type: `"form-field-*"`) - All with matching `formId`
3. **Submit button** (type: `"nav-button"` with `formId`) - Uses MODE 2

**❌ COMMON MISTAKE:** Using old `form-submit-button` type. This no longer exists!
**✅ CORRECT:** Use `nav-button` with `formId` property.

---

### Step-by-Step Form Creation

#### Step 1: Define Form Metadata (REQUIRED)

This MUST be the first block, before any form fields:

```huml
- ::
  id: "form-contact"           # ← REQUIRED: Unique form ID
  type: "form"                 # ← REQUIRED: Must be "form"
  name: "Contact Form"         # ← Optional: For editor display
  eventName: "contact_inquiry" # ← Optional: Groups submissions (default: "form_submission")
```

**Critical Properties:**
- `id` - This is what fields reference with `formId`
- `type` - Must be exactly `"form"`
- `eventName` - Groups all submissions from this form

---

#### Step 2: Add Form Fields

All fields MUST have `formId` matching the form's `id`:

```huml
# Text field
- ::
  type: "form-field-text"      # ← Field type
  formId: "form-contact"       # ← MUST match form id above!
  fieldName: "name"            # ← Key name in submission data
  label: "Your Name"           # ← Display label
  placeholder: "John Doe"      # ← Placeholder text
  required: true               # ← Validation (optional)
  css: "margin-bottom: 20px;"  # ← Styling (optional)

# Email field
- ::
  type: "form-field-email"     # ← Email type (has validation)
  formId: "form-contact"       # ← MUST match!
  fieldName: "email"
  label: "Email Address"
  placeholder: "you@example.com"
  required: true
...
# Textarea field
- ::
  type: "form-field-textarea"  # ← Multi-line text
  formId: "form-contact"       # ← MUST match!
  fieldName: "message"
  label: "Your Message"
  placeholder: "Tell us..."
...
- `form-field-text` - Single-line text
- `form-field-email` - Email with validation
- `form-field-textarea` - Multi-line text
- `form-field-checkbox` - True/false checkbox

---

#### Step 3: Add Submit Button (nav-button MODE 2)

**❌ WRONG - Don't use this (old, doesn't exist):**
```huml
# This will NOT work!
- ::
...
**✅ CORRECT - Use nav-button with formId:**
```huml
- ::
  type: "nav-button"              # ← Must be nav-button!
  content: "Submit Form"          # ← Button text
  formId: "form-contact"          # ← Triggers MODE 2 (form submission)
  targetContainerId: "thank-you" # ← Where to go after submit
  css: "background: #667eea; color: white; padding: 15px 30px; border-radius: 8px;"
```

**How MODE 2 works:**
1. User clicks button
2. System finds all fields with matching `formId`
...
- ::
  id: "thank-you"
  name: "Thank You"
  css: "padding: 40px; text-align: center;"
  children::
    - ::
      type: "heading"
      content: "✅ Thank You!"
      css: "color: green; font-size: 36px;"
...
Here's a **GUARANTEED WORKING** form template:
...
screens::
  - ::
    ...
    name: "Contact Page"
    children::
      # STEP 1: Form metadata (MUST BE FIRST!)
      - ::
        id: "form-contact"
        type: "form"
        name: "Contact Form"
      ...
      # STEP 2: Form fields (all have matching formId)
      - ::
        type: "form-field-text"
        formId: "form-contact"
        fieldName: "name"
      ...
      # STEP 3: Submit button (nav-button with formId)
      - ::
        type: "nav-button"
        content: "Send Message"
        formId: "form-contact"
        targetContainerId: "thank-you"
        css: "background: #667eea; color: white; padding: 15px; width: 100%;"
...
1. Switch to **Viewer mode** (not Builder mode)
...
#### ❌ Mistake 1: Using form-submit-button
```huml
# This doesn't exist!
- ::
  type: "form-submit-button"
...
**✅ Fix:** Use nav-button with formId:
```huml
- ::
  type: "nav-button"
  content: "Submit"
...
#### ❌ Mistake 2: Forgetting form metadata
```huml
screens::
  - ::
    id: "contact"
    children::
      # Missing form metadata block!
      - ::
        type: "form-field-text"
        formId: "form-contact"
        fieldName: "name"
...
#### ❌ Mistake 3: Mismatched formId
```huml
# Form has id "form-contact"
- ::
  id: "form-contact"
  type: "form"

# But field uses wrong formId
- ::
  type: "form-field-text"
  formId: "contact-form"  # ← WRONG! Doesn't match!
  fieldName: "name"
```

**✅ Fix:** Make formId match exactly:
```huml
- ::
  id: "form-contact"
  type: "form"
...
- ::
  type: "form-field-text"
  formId: "form-contact"  # ← Must match exactly!
  fieldName: "name"
```

---

#### ❌ Mistake 4: No targetContainerId on submit button
```huml
- ::
  type: "nav-button"
  content: "Submit"
...
**✅ Fix:** Always specify where to navigate after submit:
```huml
- ::
  type: "nav-button"
  content: "Submit"
  formId: "form-contact"
  targetContainerId: "thank-you"  # ← Required!
```
...
- [ ] Submit button is `type: "nav-button"` (NOT form-submit-button)
